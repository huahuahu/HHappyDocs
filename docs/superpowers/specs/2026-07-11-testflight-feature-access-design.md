...
在 `HDiaryConstants` 中提供一个轻量、可复用的分发来源判定器
`AppDistribution`。实现文件位于
`HDiaryLibrary/Sources/HDiaryConstants/AppEnvironment/AppDistribution.swift`，供主 App
进程内的 IAP、关于页面、诊断和其他功能模块共同使用。它不属于订阅业务策略，
因此不继续放在 `HDiaryIAP` 中；当前共享能力规模也不足以单独创建新的 Swift Package
target。

`AppDistribution` 类型、`.testFlight`、`.other` 和 `.current` 对其他模块公开。
接收明确输入的 `classify(...)` 保持模块内部可见，只用于隔离底层判定和支持
`HDiaryConstantsTests` 单元测试，避免扩大公共 API。

只有同时满足以下条件时，才判定为 `.testFlight`：
...
该 API 只承诺主 App 进程中的行为。Widget 或其他 App Extension 的 `Bundle.main`
指向扩展自身的 bundle，不能使用这套 `.current` 结果代表主 App 分发来源；扩展场景不在
本次设计范围内。
...
在 `HDiaryConstantsTests` 中为纯判定逻辑增加以下单元测试：
...
- 增加服务端收据验证。
- 在 Widget 或其他 App Extension 中直接使用 `AppDistribution.current`，或通过 App Group
  共享主 App 的分发来源。
...
- 主 App 内的非 IAP 模块导入 `HDiaryConstants` 后，可以通过
  `AppDistribution.current` 获取同一套分发来源结果，而无需依赖 `HDiaryIAP`。
