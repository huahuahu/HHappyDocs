...
private struct EntryListFooterView: View {
  ...
  var body: some View {
    ...
    case .loading:
      Text(HDocString.CloudData.syncing)
    case .allLoaded:
      Text(HDocString.CloudData.allContent)
    case .hasMore:
      Button {
        loadMoreAction()
      } label: {
        Text(HDocString.CloudData.loadMore)
      }
    case .fetchMoreFail:
      Button {
        loadMoreAction()
      } label: {
        Text(HDocString.CloudData.loadFail)
      }
    }
  }
}
